/**
 * mapSvg 5.5.9 - Interactive Map Plugin
 *
 ...
 */
 ...
 (function( $ ) {
    ...
    var defaults = {
        ...
        colors              : {background: "#eeeeee", selected: 7, hover: 3},
        regions             : {},
        viewBox             : [],
        cursor              : 'default',
        scale               : 1,
        tooltipsMode        : 'names',
        tooltips            : {show: true, mode: 'names', showForSelected: true, placement: 'bottom-right', width: 'auto', height: 'auto'},
        popovers            : {show: true, placement: 'top', width: 'auto', height: 'auto'},
        onClick             : null,
        mouseOver           : null,
        mouseOut            : null,
        disableAll          : false,
...
var mapSVG = function(elem, options){
    ...
    this.methods = {
        ...
        getPlacementCSS : function(placement){
            switch(placement){
                case 'bottom-right':
                    return {
                            'left': function(){return _data.mouse.x;},
                            'top' : function(){return _data.mouse.y + 27},
                           };
                    break;
                case 'bottom-left' :
                    return {
                            'left': function(){return _data.mouse.x - $(this).outerWidth(false) + 12;},
                            'top' : function(){return _data.mouse.y + 27;},
                           };
                    break;
                case 'bottom' :
                    return {
                            'left': function(){return _data.mouse.x - $(this).outerWidth(false)/2 + 6;},
                            'top' : function(){return _data.mouse.y + 27;},
                           };
                    break;
                case 'top' :
                    return {
                            'left': function(){return _data.mouse.x - $(this).outerWidth(false)/2;},
                            'top' : function(){return _data.mouse.y - $(this).outerHeight(false) - 7;},
                           };
                    break;                                                    
                case 'top-left' :
                    return {
                            'left': function(){return _data.mouse.x - $(this).outerWidth(false) + 1;},
                            'top' : function(){return _data.mouse.y - $(this).outerHeight(false) - 7;},
                        };
                    break;
                case 'top-right':
                    return {
                            'left': function(){return _data.mouse.x - 1},
                            'top' : function(){return _data.mouse.y - $(this).outerHeight(false) - 7;},
                        };
                    break;                                                                                            
            }            
        },
        getPlacementCSSFixed : function(pos, placement){
            var x = pos[0];
            var y = pos[1];
            switch(placement){
                case 'bottom-right':
                ...
                case 'bottom-left' :
                    return {
                            'left': function(){return x - $(this).outerWidth(false);},
                            'top' : y
                           };
                    break;
                case 'bottom' :
                    ...
                    return {
                            'left': function(){return x - $(this).outerWidth(false)/2;},
                            'top' : y
                           };
                    break;
                case 'top' :
                    return {
                            'left': function(){return x - $(this).outerWidth(false)/2;},
                            'top' : function(){return y - $(this).outerHeight(false);}
                           };
                    break;
                case 'left' :
                    return {
                            'left': function(){return x - $(this).outerWidth(false);},
                            'top' : function(){return y - $(this).outerHeight(false)/2;}
                           };
                    break;
                case 'right' :
                    return {
       ...
       regionAdd : function (_item){
            ...
            _item.default_attr['fill']         = _item.attr('fill') || 'none';
            ...
            if(_data.options.regions[name]){
                ...
                if(_data.options.regions[name].data)
                    _item.data(options.regions[name].data);
            }
        ...
        setTooltip : function (mode){
            ...
            $.extend(_data.options.tooltips, {mode: _data.options.tooltipsMode});
            ...
            _data.mapTip.css({
               ...
               'padding': '4px 7px',
               'max-width': _data.options.tooltips.width!='auto'? _data.options.tooltips.width : '600px',
               'width': _data.options.tooltips.width,
               'height': _data.options.tooltips.height,
               'pointer-events':'none'
            });

            var pl = _this.getPlacementCSS(_data.options.tooltips.placement);
            
            _data.$map.mousemove(function(e) {
                if($(_data.mapTip).is(':visible'))
                    _data.mapTip.css(pl);
            });
            

            _this.showTip = (_data.options.tooltips.mode == 'custom' ?
                function (obj){
                    if((obj.disabled) || (!_data.options.tooltips.showForSelected && _data.selected_id == obj.id))
                        return false;
                    if(obj.tooltip){
                        _data.mapTip.html(obj.tooltip);
                        _data.mapTip.show();
                    }
                }: _data.options.tooltips.mode == 'names' ?
                function (obj){
                    if((obj.disabled) || (!_data.options.tooltips.showForSelected && _data.selected_id == obj.id))
                        return false;
                    _data.mapTip.html(obj.name.replace(/_/g, ' '));
                    _data.mapTip.show();
                }: _data.options.tooltips.mode == 'combined' ?
                function (obj){
                    if(!_data.options.tooltips.showForSelected && _data.selected_id == obj.id)
                        return false;                    
                    if(obj.tooltip){
                        _data.mapTip.html(obj.tooltip);
                        _data.mapTip.show();
            ...
            _this.showPopover = function (e, content, pos){
                                    ...
                                    if(content){
                                        ...
                                        */
                                        ...
                                        _data.mapPopover.css(_this.getPlacementCSSFixed(pos, _data.options.popovers.placement));
                                        
                                        
                                        if(_data.mapPopover.is(':hidden')){
                                            _data.mapPopover.show(0,function(){
                                                $('body').on('mousedown', _this.popoverOffHandler);    
                                            });                                                                                    
                                        }
                                    }else{
                                        _this.hidePopover();
        ...
        init : function(opts, elem) {
            ...
            if(!touchDevice)
                $(window).on('mousemove',function(e){
                    _data.mouse = {'x':e.clientX + $(window).scrollLeft(), 'y':e.clientY + $(window).scrollTop()};    
                });
            ...
            $.ajax({
                ...
                success:  function(xmlData){
                    ...
                    if(!touchDevice){
                        ...
                        if(_data.options.tooltips.show)
                            funcStr += 'methods.showTip(this);';
                        if(_data.options.mouseOver)
